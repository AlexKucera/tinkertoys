#!/bin/bash
#Programmer "Ten Elite Brains", email: atqueensu@gmail.com 
#Feel free to use, share, modify, whatever the fuck u wish with the code. Spread the joy of Linux. Viva la Linux
#Option: -f     Argument: <filePath/fileName> File that will be assessed and extension of the file is corrected.
#Option: -d     Argument: <directoryPath/directoryName> All the files in the directory will be assessed and extensions of each file is corrected. 
#Option: -r     used with -d, recursively effects folder nested under provided directory.
#Option: -s 	Shows details of each file in Command Line Interface (CLI).
#Option: -v     Outputs list of corrected files in ~/fixImgExt_info.txt
#Option: -h 	Help 
checkCLData()
{
...
logEntry()
{
    if [ ! -z "$v_parameter" ]; then
        ...
        echo -e "$timeStamp\t$1" >> fixImgExt_info.txt
    fi
}
...
{
    local fPath="$1"
    if [ ! -f "$fPath" ]; then                                          #whether file exists
        echo -e "Error!\t$fPath does not exist, exiting function."
        #return 0        
    fi
...
{
    local dPath="$1"
    if [ ! -d "$dPath" ]; then                                  #whether directory exists
        echo -e "Error! \t$dPath does not exist, exiting function."
        return 0        
    fi
