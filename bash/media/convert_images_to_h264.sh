...
#
#   Usage: convert_images_to_h264.sh <path to file> [options]
# 
...
show_help() {
    cat << EOF
Usage: convert_images_to_h264.sh <path to file> [resolution] [framerate] [quality] [max_bitrate]

DIY Transcoding to h264 from image sequences
...
Examples:
    ...
    convert_images_to_h264.sh sequence.%04d.jpg 1920x1080 25 20 15000
    convert_images_to_h264.sh input.mov 1280x720 30 18 8000
...
video_codec="$(setup_h264_codec "$resolution" "$fps" "$quality" "$max_bitrate")"
audio_codec="$(setup_audio_codec "aac" "160k" "2")"

# Start encoding
echo "Starting H.264 encoding..."

# Build ffmpeg command
if [[ "${file_parts[is_sequence]}" == "true" ]]; then
    # For sequences, use the counter from the filename
    start_number="${file_parts[counter]}"
    ffmpeg_cmd="ffmpeg -y -f image2 -start_number $start_number -r $fps -i \"$sequence\" $video_codec -f mp4 \"$output_file\""
else
    # For single files
    ffmpeg_cmd="ffmpeg -y -i \"$fullpath\" $video_codec -f mp4 \"$output_file\""
fi
...
if eval "$ffmpeg_cmd"; then
    ...
    echo "✗ Error: H.264 encoding failed" >&2
    exit 1
fi